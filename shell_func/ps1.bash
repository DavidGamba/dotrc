...
function k8s_ps1 {
	K8S_CONTEXT=$(kubectl config view --minify --output 'jsonpath={.contexts[].name} {..namespace}' 2>/dev/null)
}
...
function aws_shell {
	[[ -n $SAML2AWS_PROFILE ]] && echo -ne " 󰅠 $SAML2AWS_PROFILE"
}
...
function ps1 {
	...
	local FIRST_LINE="${CB}${B_DEFAULT};${HOST_COLOR};${BOLD}${CE}\h ${CB}${FAINT};${HOST_COLOR}${CE}\D{%F %T} ${CB}${CYAN}${CE}\w${CB}${DEFAULT};${RESET}${CE} \$(smiley)${CB}${HOST_COLOR};${NORMAL}${CE} \${GIT_BRANCH} ${CB}${GREEN};${NORMAL}${CE}\${K8S_CONTEXT} \$(kubie_shell) \$(aws_shell)"
